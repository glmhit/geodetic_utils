...
void gps_callback(const sensor_msgs::NavSatFixConstPtr& msg)
{
  ...
  geometry_msgs::PointStampedPtr position_msg(new geometry_msgs::PointStamped);
  ...
  transform.setRotation(tf::Quaternion(g_latest_imu_msg.orientation.x, g_latest_imu_msg.orientation.y,
                                       g_latest_imu_msg.orientation.z, g_latest_imu_msg.orientation.w));
  p_tf_broadcaster->sendTransform(tf::StampedTransform(transform, ros::Time::now(), g_frame_id, g_tf_child_frame_id));
}
...
int main(int argc, char** argv)
{
  ...
  ros::param::param<std::string>("~frame_id", g_frame_id, "world");
  ros::param::param<std::string>("~tf_child_frame_id", g_tf_child_frame_id, "gps_receiver");
